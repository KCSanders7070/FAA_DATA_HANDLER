    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (parseFaaCifp)
            {
                ...
                // KeepRawRecord stores the full 132-character source line on every model. It costs about
                // 52 MB across for a cycle and makes any odd value traceable back to its source line.
                var cifpParseOptions = new CifpParseOptions
                {
                    KeepRawRecord = false,
                    ThrowOnUnknownRecord = false,
                };
                ...
                if (!cifpParseReport.IsClean)
                {
                    Console.WriteLine("WARNING: the CIFP file contained lines this parser did not recognize.");
                    Console.WriteLine("         See the samples above before trusting the parsed output.");
                }

                // Cross-check the supplier's own record count against what was actually parsed.
                HeaderInfoCifpDataModel? header = cifpDataCollections.HeaderInfo
                    .Find(h => h.HeaderNumber == "01");

                if (header?.RecordCount is int declaredRecordCount)
                {
                    int parsedRecordCount = cifpParseReport.TotalLines
                                          - cifpParseReport.HeaderRecords
                                          - cifpParseReport.BlankLines;

                    if (parsedRecordCount != declaredRecordCount)
                    {
                        Console.WriteLine($"WARNING: header declares {declaredRecordCount:N0} records "
